...
7. Review the synchronization pull request and merge it with a **merge commit**. Do not squash it; preserving the release commit ancestry keeps `dev` aligned with the release history on `main`. The workflow never pushes to or merges `dev` directly.
