...
The adapter uses Hyperindex's PostgreSQL schema after the external-label and record-timeline migrations. The directly consumed columns are:
...
Missing rows produce a smaller feed. They do not make readiness fail, and readiness does not verify collection filters or ingestion completeness.
...
The query does not read `actor`. Hyperindex removes records and actor rows when an identity is explicitly deleted, deactivated, suspended, or taken down. Actors without an `actor` row remain eligible; an inactive identity naturally produces no feed events after Hyperindex purges its source records.
...
The deployment role needs only schema usage and `SELECT` on the two runtime tables:
...
Before production traffic, run `src/feed/feed-query.sql` through `EXPLAIN (ANALYZE, BUFFERS)` with production-shaped data for small and large followed scopes, evaluator expansion, organization-quality filtering, and Hyperboard events.
